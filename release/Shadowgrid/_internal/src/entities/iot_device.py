...
def _get_iot_sprite(device_type: str) -> Optional[pygame.Surface]:
    if device_type not in _IOT_SPRITES:
        ...
        # Case fallback check (z.B. PC.png vs pc.png)
        if not os.path.exists(path) and device_type == "pc":
            path = os.path.abspath(os.path.join("src", "gfx", "PC.png"))
...
class IoTNode:
    """
    IoT Surveillance Devices & Nodes (16x16 px Sprites):
    - Watcher (watcher.png): Gedockt am Grid-Rand.
    - Listener (listener.png): Gedockt am Grid-Rand.
    - Guard Spot (guard.png): Gedockt am Grid-Rand. Sentinel-Spawnplatz (nicht hackbar).
    - PC Node (PC.png / pc.png): Gedockt am Grid-Rand. Kommt häufig vor, hackbar.
    - Proxy Spot (proxy.png): Liegt auf dem Pfad. Schutzzone (nicht hackbar).
    - Scanner (scanner.png): Liegt auf dem Pfad. Verlangsamt passierende Entitäten.
    """
    def __init__(self, grid_x: int, grid_y: int, device_type: str = "watcher", dock_tile: Optional[Tuple[int, int]] = None, dock_dir: Optional[Tuple[int, int]] = None) -> None:
        ...
        elif dev in ("proxy", "sanctuary"):
            self.device_type = "proxy"
        elif dev in ("guard", "guard_spot", "spawn"):
            self.device_type = "guard"
        elif dev in ("pc", "computer", "terminal"):
            self.device_type = "pc"
        elif dev in ("medic", "doctor", "clinic"):
            self.device_type = "medic"
        elif dev in ("merchant", "vendor", "shop"):
            self.device_type = "merchant"
        else:
        ...
        # Datenzähler: PCs starten mit 10 Daten. Watcher/Listener mit 0 (zählen vorbeilaufende Civs)
        if self.device_type == "pc":
            self.data_count = 10
        elif self.device_type in ("watcher", "listener"):
            self.data_count = 0
        else:
            self.data_count = 0
        ...
        # Spawn Steuerung für Guard Nodes
        self.max_spawns = 1
        self.spawned_count = 0
        self.spawn_cooldown = random.uniform(4.0, 12.0)
        ...
        self.jack_detected = False
        self.pulse_timer = random.uniform(0.0, 3.14)

    @property
    def is_hackable(self) -> bool:
        return (self.device_type in ("watcher", "listener", "pc")) and (self.data_count > 0 or self.jack_detected)
    ...
    def draw(self, surface: pygame.Surface, camera_offset_x: float, camera_offset_y: float, zoom: float, override_color: Optional[Tuple[int, int, int]] = None, show_target_anim: bool = True) -> None:
        ...
        if self.dock_tile and self.dock_dir and self.device_type in ("watcher", "listener", "guard", "pc"):
        ...
        if sprite:
            ...
            if self.device_type == "medic":
                color = (0, 255, 150)
            elif self.device_type == "merchant":
                color = (255, 180, 0)
            else:
            ...
            if self.device_type == "medic":
                symbol_surf = font_mgr.render("+", color=color, size="tiny")
                surface.blit(symbol_surf, (cx - symbol_surf.get_width() // 2, cy - symbol_surf.get_height() // 2))
            elif self.device_type == "merchant":
                symbol_surf = font_mgr.render("€", color=color, size="tiny")
                surface.blit(symbol_surf, (cx - symbol_surf.get_width() // 2, cy - symbol_surf.get_height() // 2))

        # 3. Ziel- / Hover-Anzeige
        if show_target_anim:
            if self.is_pursued:
                ...
                half = frame_size // 2
                hover_color = (0, 255, 0) if self.is_hackable else (120, 120, 120)
                pygame.draw.rect(surface, hover_color, (cx - half, cy - half, frame_size, frame_size), max(1, int(1.5 * zoom)))

        # 4. Data-Count & Jack-Sighting Indikator zeichnen (immer sichtbar für Watcher, Listener, PC)
        if self.device_type in ("watcher", "listener", "pc") and show_target_anim:
            from src.ui.font_manager import font_mgr
            color = (0, 255, 200) if self.data_count > 0 else (100, 150, 150)
            label_str = f"+{self.data_count}"
            if self.device_type == "watcher" and self.jack_detected:
                label_str += " [!]"
                color = (255, 50, 50)
            cnt_txt = font_mgr.render(label_str, color=color, size="tiny")
            surface.blit(cnt_txt, (cx - cnt_txt.get_width() // 2, cy - int(12 * zoom)))

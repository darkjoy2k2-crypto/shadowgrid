...
class ActionMenu(UIWindow):
    ...
    def open(self, x: int, y: int, target: Any = None) -> None:
        if not self.active:
            self.active = True
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        if getattr(self, 'target', None):
            t = self.target
            if hasattr(t, 'name'):
                title_str = t.name[:14].upper()
            elif hasattr(t, 'device_type'):
                dc = getattr(t, 'data_count', 0)
                title_str = f"{t.device_type.upper()} [+{dc}]"
        title = font_mgr.render(title_str, color=GREEN_TERMINAL, size="tiny")

...
class HUD:
    ...
    def draw(self, surface: pygame.Surface, values: Dict[str, float], flip_x: bool = False, thought_text: str = "JACK // ONLINE", feed_lines: Optional[List[str]] = None) -> None:
        ...
        # Live Fantasie Code Feed Zeilen direkt darunter rendern
        if feed_lines:
            feed_y = text_y + len(lines) * 12 + 2
            for f_idx, f_line in enumerate(feed_lines[:2]):
                # Kürzen falls zu lang für Terminal Box
                f_surf = font_mgr.render(f_line[:32], color=(0, 180, 110), size="tiny")
                surface.blit(f_surf, (text_x, feed_y + f_idx * 10))
